...
CITIES = {
    ...
    # Добавленные города (в алфавитном порядке)
...
DB_PATH = "/opt/airflow/data/weather.db" # Абсолютный путь в контейнере
...
def get_weather_data(lat, lon, city_name):
    ...
    df_daily = df_daily.with_columns(
        (
            -pl.col("total_precip") * 3          # штраф за осадки
            - pl.col("avg_wind") * 0.7           # штраф за ветер
            - (pl.col("avg_temp") - 20).abs() * 0.6  # отклонение от идеальной температуры
        ).alias("comfort_index")
    )
